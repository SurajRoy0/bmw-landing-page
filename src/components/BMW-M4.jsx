...
export default function ModelBMW({ headlightsOn = false, driverDoorOpen = false, passengerDoorOpen = false, ...props }) {
  const group = useRef()
  const driverDoorRef = useRef()
  const passengerDoorRef = useRef()
  const { nodes, materials, animations } = useGLTF('/models/bmw_m4_competition.glb')
  const { actions } = useAnimations(animations, group)

  // Animate driver door open/close
  useEffect(() => {
    if (!driverDoorRef.current) return
    gsap.to(driverDoorRef.current.rotation, {
      z: driverDoorOpen ? -0.65 : 0,
      duration: 1.2,
      ease: 'power2.inOut',
    })
  }, [driverDoorOpen])

  // Animate passenger door open/close
  useEffect(() => {
    if (!passengerDoorRef.current) return
    gsap.to(passengerDoorRef.current.rotation, {
      z: passengerDoorOpen ? 0.65 : 0,
      duration: 1.2,
      ease: 'power2.inOut',
    })
  }, [passengerDoorOpen])
